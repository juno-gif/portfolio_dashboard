...
export default function HeroSection({ summary, onRefresh, isRefreshing }: HeroSectionProps) {
  const todayPositive = (summary?.todayGainAmount ?? 0) >= 0;
  const totalPositive = (summary?.totalGainAmount ?? 0) >= 0;
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4 text-sm text-muted-foreground">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <span className="font-medium text-foreground">
            USD/KRW {(summary?.exchangeRate ?? 0).toLocaleString('ko-KR', { maximumFractionDigits: 0 })}
          </span>
      ...
      <div className="grid grid-cols-2 gap-4 sm:gap-6">
        ...
        <div>
          ...
          <p className={`text-2xl sm:text-4xl font-bold ${todayPositive ? 'text-green-500' : 'text-red-500'}`}>
            {todayPositive ? '+' : '-'}₩{Math.abs(summary?.todayGainAmount ?? 0).toLocaleString('ko-KR', { maximumFractionDigits: 0 })}
          </p>
          <p className={`text-lg sm:text-2xl font-semibold mt-1 ${todayPositive ? 'text-green-500' : 'text-red-500'}`}>
            {formatRate(summary?.todayGainRate ?? 0)}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-xl sm:text-2xl font-bold">
            ₩{(summary?.totalEval ?? 0).toLocaleString('ko-KR', { maximumFractionDigits: 0 })}
          </p>
          <p className="text-sm text-muted-foreground mt-1">
            투자원금 ₩{formatKRW(summary?.totalCost ?? 0)}
          </p>
          <p className={`text-sm font-medium mt-1 ${totalPositive ? 'text-green-500' : 'text-red-500'}`}>
            전체 {totalPositive ? '+' : '-'}₩{Math.abs(summary?.totalGainAmount ?? 0).toLocaleString('ko-KR', { maximumFractionDigits: 0 })} ({formatRate(summary?.totalGainRate ?? 0)})
          </p>
        </div>
      </div>
    </div>
